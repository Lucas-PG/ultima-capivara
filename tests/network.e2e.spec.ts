...
test('host and guest exchange lobby, gameplay, recovery, and close over local PeerJS', async ({ browser }) => {
  ...
  try {
    ...
    const roomCode = await host.evaluate(async settings => {
      ...
      w.net = { rooms: [], starts: [], inputs: [], actions: [], players: [], snapshots: [], closed: [] };
      w.session = new RoomSession({
        ...
        events: () => {}, error: (v: unknown) => { throw new Error(String(v)); }, closed: (v: unknown) => w.net.closed.push(v),
      });
      await w.session.host({ name: 'Host', color: '#bd8956' }, settings);
    ...
    const guestId = await guest.evaluate(async code => {
      ...
      try { await w.session.join(code, { name: 'Guest', color: '#8f6347' }); }
