...
const MAG_GRIP: Record<WeaponId, Point> = {
  ...
  shotgun: [-.055, -.14, -.09], dmr: [-.074, -.29, -.12], sniper: [-.074, -.29, -.12],
  machete: [0, 0, 0], slingshot: [-.025, .045, .21],
};
const ACTION_GRIP: Record<WeaponId, Point> = {
  pistol: [-.068, .045, .10], smg: [-.035, .02, -.015], m4: [-.025, .065, .08],
  shotgun: SUPPORT_PALM.shotgun, dmr: [.10, -.02, .045], sniper: [.155, -.03, .063],
  machete: [0, 0, 0], slingshot: SUPPORT_PALM.slingshot,
};
export interface ReloadPose {
  mag: number; action: number; actionRoll: number;
  handX: number; handY: number; handZ: number; handRoll: number;
  lift: number; pitch: number; roll: number; bump: number; prop: number;
}
export const createReloadPose = (): ReloadPose => ({ mag: 0, action: 0, actionRoll: 0,
  handX: 0, handY: 0, handZ: 0, handRoll: 0, lift: 0, pitch: 0, roll: 0, bump: 0, prop: 0 });
...
export function sampleReload(id: WeaponId, progress: number, out: ReloadPose): ReloadPose {
  ...
  if (id === 'machete' || p === 0 || p === 1) {
    for (const key of Object.keys(out) as (keyof ReloadPose)[]) out[key] = 0;
    return out;
  }
  const shell = id === 'shotgun', pebble = id === 'slingshot', single = shell || pebble;
  const hold = phase(p, .015, cue.grab) * (1 - phase(p, cue.close, 1));
  const withdraw = phase(p, cue.grab, cue.out) * (1 - phase(p, cue.insert, cue.seat));
  const rack = pulse(p, cue.seat + .06, cue.rack, cue.close);
  const transfer = phase(p, cue.seat + .025, cue.rack - .025);
  ...
  out.mag = single ? 0 : -withdraw * (id === 'pistol' ? .26 : .32);
  out.action = single ? 0 : rack * (id === 'sniper' ? .095 : id === 'pistol' ? .052 : .065);
  out.actionRoll = id === 'sniper' ? -.65 * pulse(p, cue.seat + .035, cue.seat + .09, cue.close) : 0;
  if (single) {
    ...
    out.handX = ((contact[0] - palm[0]) * grasp * (1 - transfer) + (action[0] - palm[0]) * transfer) * returnHand;
    out.handY = (((contact[1] - palm[1]) * grasp + out.mag) * (1 - transfer) + (action[1] - palm[1]) * transfer) * returnHand;
    out.handZ = ((contact[2] - palm[2]) * grasp * (1 - transfer) + (action[2] - palm[2] + out.action) * transfer) * returnHand;
    out.handRoll = ((id === 'pistol' ? -.2 : -.12) * (1 - transfer) + (id === 'sniper' ? .5 : -.32) * transfer) * hold;
    out.prop = 0;
  }
