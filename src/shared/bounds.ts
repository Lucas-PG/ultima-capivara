...
import type { Mode, Vec3, WorldSpec } from './types';
...
export function boundaryFeedback(pos: Pick<Vec3, 'x' | 'z'>, world: Pick<WorldSpec, 'size' | 'walkways'>, mode?: Mode): BoundaryFeedback | null {
  ...
  if (mode !== 'deathmatch' && world.walkways?.some(deck => pos.x >= deck.min.x && pos.x <= deck.max.x && pos.z >= deck.min.z && pos.z <= deck.max.z)) return null;
  const ocean = world.size / 2 - 7;
  const limits = mode === 'deathmatch' ? ARENA : { minX: -ocean, maxX: ocean, minZ: -ocean, maxZ: ocean };
  ...
  return { x: dx / distance, z: dz / distance, strength: Math.min(1, distance / 3),
    message: mode === 'deathmatch' ? 'Volte para a Vila' : 'A correnteza está forte. Volte para a ilha' };
}
