...
export class GameUI {
  ...
  private victory(snapshot: WorldSnapshot) {
    ...
    const title = won ? br ? 'Última de pé!' : snapshot.config.mode === 'corrente' ? 'Fechou a corrente!' : 'Dona da correria!' : br ? 'Não foi dessa vez' : winners.length ? `${names} ${winners.length > 1 ? 'levaram' : 'levou'}` : 'Ninguém levou';
