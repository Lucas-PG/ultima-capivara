...
export function createPaintedWeaponAtlas(colors: readonly number[]): THREE.DataTexture {
  const width = 1024, height = 1024, pixels = new Uint8Array(width * height * 4);
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
    const column = Math.floor(x / 32), u = (x % 32 + .5) / 32, v = (y + .5) / height;
