...
Baths and trampolines use the exported `interaction.surfaceY` and `interaction.radius`, transformed by the kit placement. Face their entrance toward a walking route. Reserve their surface from planting, supplies, pickups and spawns, keep open sky above the bounce pad, and prove a walk-in approach from outside its footprint. Missing interaction metadata omits both the piece and the gameplay site.
