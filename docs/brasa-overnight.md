...
The source checkpoint is integrated with verified `ab115920`. Its full code gate passed, but reload visuals require the revision below before landing. Reload, melee and transition ownership was handed off by Cena; Oficina confirmed the current palm anchors and identity part roles will survive the new paw/machete geometry. Production gameplay durations, movement, network state and damage are unchanged.
...
FP visual preflight: 131 motion frames at 720p/1080p (DPR 1), Low and reduced motion have no browser errors. Fourteen eight-frame sheets include all seven reloads, both swings, confirmed contact and four transitions. Magazine withdrawal lifts the receiver to keep the grasp above the HUD. The current machete is framed lower and farther away without changing its authored root. Zero-time redraws preserve the last swept trail segment. The QA fixture completes its displayed ammo state at the deadline and waits for the HUD cadence before capturing.

The bakery at (-43,-29) was walked using real shared `moveActor` in the runtime: front threshold, furnished room, rear exit, return and near-wall approach all remained grounded at the actual .11 m floor and returned to terrain without clipping or a dead end. Indoor reload and wall-lowering captures were reviewed. The existing tall-house stair gap is Mapa/Oficina's separate matched-geometry gate; this pass does not claim upper-floor repair. Reload exposure also revealed pre-existing detached SMG sights/stock fittings; Oficina confirmed source repairs for her pending paw/weapon export. New paw art and those geometry repairs are not in this animation checkpoint.

Exact settled plaza16 Medium720/DPR1 preflight: 1,545,859 triangles, 262 draws, 60.00 FPS, p95 18.6 ms, exactly 180 draws over 180 RAFs. Browser and dev5195 closed before the sequential full gate. Evidence: `A-brasa-fp-strip-*`, per-phase `A-brasa-fp-reload-*`, `A-brasa-fp-swing-*`, `A-brasa-fp-walk-*`, `A-brasa-fp-indoor-reload-720.png`, `A-brasa-fp-near-wall-720.png` and `A-brasa-fp-plaza16-720.png`.


### FP v1 gate result and required revision

Evidence-only runtime `fbe6094` passed check, 571 unit tests (85 files, max two workers), production build and Chromium 4/4 in 1.3 minutes. It is **not landable**: the director accepted swing timing but rejected reload readability. The raised receiver dominates the right side, the racking paw obscures the weapon, and withdrawing then returning the same magazine does not read as an exchange. Smaller, steadier receiver framing, a distinct old/new-mag handoff and a shorter real-handle rack reach require fresh strips. Pistol, shell and sniper contacts must receive the same review.

All Brasa browser/dev processes and ports 5174/9001/5195 closed before direct END to Mapa, then Cena. Mapa was told to start from verified `ab115920`, excluding this candidate. Reload revision continues source-only during the queue and will be reviewed against Oficina's pending paw/mag asset. The approved swing timing and transition work remain isolated in this source checkpoint; no partial landing is claimed.

### Reload v2 source checkpoint, awaiting new paw export and motion review

The receiver now moves slightly forward with a 20-degree cant and at most a 7.5 cm lift. A separate instance of the authored magazine is pulled out and released; a replacement rises from below the frame, pauses visibly below the well, then seats with the existing timed bump. The extra instance shares source geometry/material and disappears after the exchange. The support forearm pivots down around the unchanged palm while reaching the real action, keeping the sleeve away from the lens. The sniper paw follows the turning bolt knob before the short pull; a full-size shell seats through Oficina's supplied underside mouth at (0, -.129, -.094).

Typecheck and 48 focused tests pass sequentially with one worker: 24 timing/audio intents, 15 view lifecycle intents and nine new checks through the real decoded GLB and WeaponView. They protect visible receivers, screen-readable separated magazines, forearm clearance, cancellation/respawn and the shell contact. The new projection test caught a replacement magazine still below the frame at the insertion beat; the approach and seating curves were corrected. These CPU checks do not establish visual approval. No build, browser or full suite ran during Mapa's slot. New paw/machete geometry and fresh v2 motion strips remain required; v1 evidence is preserved for comparison.

### Endgame fallback prepared

If the final-paw reload-v2 strips need another revision, use the isolated old-reload fallback. It restores the verified 50136d5 reload transforms and audio schedule, removes the new magazine/round props and reload curves, and keeps the blade swing plus transition work. Typecheck and 18 focused inspect, ADS, melee timing and impact checks pass sequentially with one worker. No final-paw pixels or full gate are claimed yet. The primary reload-v2 branch is unchanged.
